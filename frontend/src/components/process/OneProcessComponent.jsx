...
import { sendTodoTest } from "../../fetch";
import { MainContext } from "../../App";
function OneProcessComponent({ el }) {
  const [modal, setModal] = useState(false);
  const { setObj } = useContext(MainContext);
  const sendTest = async (arg) => {
    const response = await sendTodoTest(arg);
    setObj(response);
  };
  return (
...
export default OneProcessComponent;
